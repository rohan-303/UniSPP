...
output_dir = "/home/careinfolab/Dr_Luo/Rohan/UniSPP/Results/With_UniSPP/MLP"
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, act_fn):
        super().__init__()
        layers = []
...
def evaluate_mlp(subgraph, df_train, df_test, base_features, mlp_params):
    ...
    model = MLP(input_dim=X_train.shape[1],
                ...
                act_fn=mlp_params["act_fn"]).to(device)
...
def unispp_optimize_with_phi(df_train, df_test, edges, phi, feature_index, base_features, mlp_params, trials, samples_per_trial, num_edges, lr):
    ...
    for _ in range(trials):
        for _ in range(samples_per_trial):
            ...
            mae, rmse, nrmse, losses = evaluate_mlp(subgraph, df_train, df_test, base_features, mlp_params)
...
def objective(trial):
    ...
    mae, rmse, nrmse, subgraph, _ = unispp_optimize_with_phi(
        df_train, df_test, edges, phi, feature_index, features, mlp_params,
