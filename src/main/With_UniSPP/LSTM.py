...
output_dir = "/home/careinfolab/Dr_Luo/Rohan/UniSPP/Results/With_UniSPP/LSTM"
...
def evaluate_lstm(subgraph, df_train, df_test, base_features, lstm_params):
    ...
    model = LSTMModel(input_dim=X_train_tensor.shape[2],
                      hidden_dim=lstm_params["hidden_dim"],
                      output_dim=1,
                      num_layers=lstm_params["num_layers"],
                      act_fn_name=lstm_params["act_fn"]).to(device)
...
def unispp_optimize_with_phi(df_train, df_test, edges, phi, feature_index, base_features, lstm_params, trials, samples_per_trial, num_edges, lr):
    ...
    for _ in range(trials):
        for _ in range(samples_per_trial):
            ...
            mae, rmse, nrmse, losses = evaluate_lstm(subgraph, df_train, df_test, base_features, lstm_params)
...
def objective(trial):
    ...
    mae, rmse, nrmse, subgraph, _ = unispp_optimize_with_phi(
        df_train, df_test, edges, phi, feature_index, features, lstm_params,
