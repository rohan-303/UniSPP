...
output_dir = "/home/careinfolab/Dr_Luo/Rohan/UniSPP/Results/With_UniSPP/RNN"
...
def evaluate_rnn(subgraph, df_train, df_test, base_features, rnn_params):
    ...
    model = RNNModel(input_dim=X_train_tensor.shape[2],
                     hidden_dim=rnn_params["hidden_dim"],
                     output_dim=1,
                     num_layers=rnn_params["num_layers"],
                     act_fn_name=rnn_params["act_fn"]).to(device)

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=rnn_params["lr"])

    epoch_losses = []
    for epoch in range(rnn_params["epochs"]):
        model.train()
        optimizer.zero_grad()
...
def unispp_optimize_with_phi(df_train, df_test, edges, phi, feature_index, base_features, rnn_params, trials, samples_per_trial, num_edges, lr):
    ...
    for _ in range(trials):
        for _ in range(samples_per_trial):
            ...
            mae, rmse, nrmse, losses = evaluate_rnn(subgraph, df_train, df_test, base_features, rnn_params)
...
def objective(trial):
    rnn_params = {
        ...
        "act_fn": trial.suggest_categorical("act_fn", ["relu", "tanh"]),
    ...
    mae, rmse, nrmse, subgraph, _ = unispp_optimize_with_phi(
        df_train, df_test, edges, phi, feature_index, features, rnn_params,
