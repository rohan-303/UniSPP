...
output_dir = "/home/careinfolab/Dr_Luo/Rohan/UniSPP/Results/Without_UniSPP/RNN"
...
class RNNModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, act_fn_name):
        super().__init__()
        self.rnn = nn.RNN(input_dim, hidden_dim, num_layers, batch_first=True, nonlinearity=act_fn_name)
...
def objective(trial):
    ...
    model = RNNModel(X_train_tensor.shape[2], hidden_dim, 1, num_layers, act_fn).to(device)
...
final_model = RNNModel(X_train_tensor.shape[2],
                       best["hidden_dim"],
                       1,
                       best["num_layers"],
...
epoch_losses = []
final_model.train()
for epoch in range(best["num_epochs"]):
    total_loss = 0.0
    for xb, yb in loader:
